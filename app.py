...
def extract_pages_texts(ocr_json: Dict[str, Any]) -> List[str]:
    """Return list[str] where each element is one page's text. Prefers pages[].markdown."""
...
def clean_markdown(md: str) -> str:
    md = md.replace("\r\n", "\n").replace("\r", "\n")
    md = md.replace("Student/\nSubject", "Student/Subject")  # example header fix
    return md
...
def pages_to_markdown(pages_text: List[str], insert_page_breaks: bool) -> str:
    parts: List[str] = []
...
def build_docx_with_pandoc(md_text: str) -> bytes:
...
if uploaded is not None:
    if not uploaded.name.lower().endswith(".pdf"):
        ...
        md_text = pages_to_markdown(pages_text, insert_page_breaks=st.session_state.get("opt_page_breaks", True))
        ...
        with st.spinner("Building DOCX..."):
            ...
            if want_pandoc or force_pd or has_math_or_tables:
                tried_pandoc = True
                try:
                    docx_bytes = build_docx_with_pandoc(md_text)
                    pd_ok = True
                except Exception as e:
